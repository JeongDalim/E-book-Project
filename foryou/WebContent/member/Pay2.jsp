...
<script type="text/javascript">
	...
	function cardsend() {
		...
		card.action="Member?cmd=MemberPayInsert";
		card.submit();
	...
	function banksend() {
		...
		form.action="Member?cmd=MemberPayInsert";
		form.submit();
		alert("Payment information has been registered.");
		form.action="Rent?cmd=bookRent&idx=${idx}";
		form.submit();
		opener.window.location.href="Book?cmd=view&idx=${idx}";
		self.close();
	}
</script>
</head>
			...
			<table class="Login_tab_P">
				<tr>
					<td><input type="text" id="bank" name="bank"
						placeholder="계좌번호 를 입력하세요." maxlength="20"></td>
				</tr>
				<tr>
